  ...
  plugins: [
    {
      ...
      closeBundle() {
        ...
        copyFileSync("assets/components/header.html", "dist/assets/components/header.html");
        copyFileSync("assets/components/footer.html", "dist/assets/components/footer.html");

        const imagesDirs = ["animaCards", "smalss"];
        imagesDirs.forEach(dir => {
          mkdirSync(`dist/assets/images/${dir}`, { recursive: true });
          readdirSync(`assets/images/${dir}`).forEach(file => {
            copyFileSync(`assets/images/${dir}/${file}`, `dist/assets/images/${dir}/${file}`);
          });
        });
